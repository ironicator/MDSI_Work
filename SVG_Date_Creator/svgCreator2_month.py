...
while current_date <= end_date:
    ...
    month_folder = os.path.join(output_folder, f'{current_date.month:02}')
    os.makedirs(month_folder, exist_ok=True)
    file_path = os.path.join(month_folder, f'{year}{current_date.month:02}{day}.svg')
...
output_paths_folder = os.path.join(script_dir, f"output/{year}")
os.makedirs(output_paths_folder, exist_ok=True)
inkscape_path = r"C:/Program Files/Inkscape/bin/inkscape.exe"

# Define the PowerShell command as a multi-line string
powershell_script = f'foreach ($file in Get-ChildItem -Path "{output_folder}" -Recurse -Filter "*.svg") ' \
...
# Loop through each subdirectory and file in output_folder
for root, dirs, files in os.walk(output_folder):
    for filename in files:
        if filename.endswith("_out.svg"):
            ...
            # Get the relative path of the subdirectory within output_folder
            relative_subdir = os.path.relpath(root, output_folder)
            
            # Create the destination subdirectory path within output_paths_folder
            destination_subdir = os.path.join(output_paths_folder, relative_subdir)
